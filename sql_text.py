...
updateOptionValueStr = "replace into option select contract,instrument,date,commodity,callPut,account,last_price,today_price,margin,long_quantity,long_notional,short_quantity,short_notional,{0},{1} from option where contract = '{2}' and date={3} and account like '%{4}%'"
...
addOptionValueStr = "replace into settlement select p.account, p.date, p.lastDate, p.today, p.equity + sum(long_value)-sum(short_value) as equity, realised, unrealised,commission,p.margin,deposit,return from option o join settlement p on o.account=p.account and o.date=p.date where o.account in ({0}) and o.date={1} group by o.date,o.account"
...
futureDefinition = '''CREATE TABLE future
           ...
           today_price        REAL     NULL,
           margin             REAL     NULL,
           unrealised         REAL     NULL,
...
inventoryDefinition = '''CREATE TABLE inventory
           ...
           date               INT      NOT NULL,
           PRIMARY KEY (date, id));'''
tradeDefinition = '''CREATE TABLE trade
           (instrument        TEXT     NOT NULL,
           account            TEXT     NOT NULL,
           price              REAL     NULL,
           quantity           REAL     NULL,
           ...
           realised_pl        REAL     NULL,
           Premium            REAL     NULL,
           id                 TEXT     NOT NULL,
           date               INT      NOT NULL,
           PRIMARY KEY (date, id));'''
Definitions = [tradeDefinition, inventoryDefinition, commodityDefinition, marketDataDefinition, profitLossDefinition, futureDefinition, optionDefinition, greeksDefinition, settlementDefinition]
